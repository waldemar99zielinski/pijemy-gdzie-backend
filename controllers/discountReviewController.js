...
exports.review = async (req, res, next) => {
  try {
    ...
    if (
      !req.body.review ||
      (req.body.review != "like" && req.body.review != "dislike")
    ) {
      return next(
        new ErrorHandler("Wrong review parameters: " + req.body.review, 404)
      );
    }
    ...
    const reviewToFind = await DiscountReview.findById(
      discountToFind.discountReview
    );
    if (!reviewToFind) {
    ...
    let userReviewToFind = await UserReview.findOne({
      userId: userId,
    ...
    if (!userReviewToFind) {
      ...
      updatedReview = await DiscountReview.findByIdAndUpdate(
        reviewToFind.id,
        reviewToFind,
        { new: true }
      );

      userReviewToFind = await UserReview.create({
        userId: userId,
        discountId: discountToFind.id,
        review: review,
      });
    ...
    //user has already reviewed this discount
      ...
      if (oldReview == review) {
        ...
        const userRev = await UserReview.findOneAndDelete({
          userId: userId,
          discountId: discountToFind.id,
        });
      } else {
        ...
        await UserReview.findOneAndUpdate(
          { userId: userId, discountId: discountToFind.id },
          { $set: { review: review } }
        );
      }
    }
